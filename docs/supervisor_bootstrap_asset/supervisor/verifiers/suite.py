...
class VerifierSuite:
    ...
    def run(self, verify_checks, context: dict) -> dict:
        results = []
        for check in verify_checks:
            ...
            if check_type == "command":
                results.append(self.command_verifier.run(payload))
            elif check_type == "artifact":
                results.append(self.artifact_verifier.run(payload))
            elif check_type == "git":
                results.append(self.git_verifier.run(payload))
